...
```
root@node1:~/policies# kubectl exec -n dev dev-web2-5cd468cdf5-sjnmb  -- curl -s -m 1 hello-node.default:8080
command terminated with exit code 28
```

Создадим политику, которая разрешает доступ с namespace dev только из dev-web1 пода к hello-node
...
Проверяем
```
root@node1:~/policies# kubectl exec -n dev dev-web1-5f45c88676-vl6hm  -- curl -s -m 1 hello-node.default:8080
CLIENT VALUES:
client_address=10.233.105.5
command=GET
real path=/
