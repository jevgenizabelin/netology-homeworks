...
k-instance                 : ok=4    changed=0    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
```
10. В этом плейбуке выполняются 3 плея:
   Install Elasticsearch, Install Kibana, Install Filebeat. 
    В плеях Install Elasticsearch, Install Kibana и Install Filebeat скачиваем на удаленных хостах в директорию /tmp необходимые версии deb пакетов,
    прописанные в переменной `elk_stack_version` в файлах по пути /inventory/prod/group_vars/.
...
11. https://github.com/jevgenizabelin/netology-ansible3
